...
def digest(path: Path) -> str:
    ...
    return h.hexdigest()
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--output", type=Path, default=ROOT / "dist" / "resource.ilunyupack"); parser.add_argument("--release-output", type=Path); args = parser.parse_args()
    config = read(ROOT / "resource.json"); exercises = []
    ...
    with tempfile.TemporaryDirectory(prefix="ilunyu-package-") as directory:
        build, content = Path(directory), Path(directory) / "content"
        summary = lambda item: {key: item[key] for key in ("id", "title", "year", "source", "type", "grade", "number", "score", "month") if key in item}
        write(content / "index.json", {"formatVersion": 2, "exercises": [summary(item) for item in exercises]})
        write(content / "search.json", {"formatVersion": 2, "exercises": [{**summary(item), "question": item.get("question", [])} for item in exercises]})
        for item in exercises: write(content / "items" / f"{item['id']}.json", item)
        files = {path.relative_to(build).as_posix(): f"sha256:{digest(path)}" for path in sorted(content.rglob("*.json"))}
        manifest = {"packageFormat": 1, "contentSchema": 1, "packageId": config["packageId"], "kind": config["kind"], "name": config["name"], "versionName": config["versionName"], "versionCode": config["versionCode"], "minAppVersionCode": config["minAppVersionCode"], "sourceRepository": config.get("sourceRepository", ""), "license": config.get("license", ""), "files": files}; write(build / "manifest.json", manifest)
